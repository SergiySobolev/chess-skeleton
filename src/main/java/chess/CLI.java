...
public class CLI {
    ...
    private void listMoves() {
        ...
        for(Move move : gameState.findAllPossibleMovesForWhitePlayer()) {
            writeOutput(move.toString());
        }
        writeOutput("Black's Possible Moves:");
        for(Move move : gameState.findAllPossibleMovesForBlackPlayer()) {
            writeOutput(move.toString());
        }
    }
